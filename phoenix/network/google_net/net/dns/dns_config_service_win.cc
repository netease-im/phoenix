...
base::scoped_ptr<IP_ADAPTER_ADDRESSES, base::FreeDeleter> ReadIpHelper(ULONG flags) {
  base::ThreadRestrictions::AssertIOAllowed();

  base::scoped_ptr<IP_ADAPTER_ADDRESSES, base::FreeDeleter> out;
...
//https://docs.microsoft.com/zh-cn/windows/win32/api/winnls/nf-winnls-idntoascii?redirectedfrom=MSDN
 //Windows XP, Windows Server 2003: 404
...
//  auto* ASCIICharStr = new base::char16[widestr.length() * 2];
//  auto ASCIICharStr_Length = IdnToAscii(IDN_RAW_PUNYCODE, widestr.data(), widestr.length(), ASCIICharStr, 0);
//  if (ASCIICharStr_Length == 0)
//	  return false;
//  else
//	punycode.Append(ASCIICharStr, ASCIICharStr_Length);
...
HostsParseWinResult AddLocalhostEntries(DnsHosts* hosts) {
  ...
  base::scoped_ptr<IP_ADAPTER_ADDRESSES, base::FreeDeleter> addresses =
...
// static
base::scoped_ptr<DnsConfigService> DnsConfigService::CreateSystemService() {
  return base::scoped_ptr<DnsConfigService>(new internal::DnsConfigServiceWin());
}
