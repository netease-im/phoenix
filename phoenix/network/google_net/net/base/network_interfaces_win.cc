...
base::scoped_ptr<WLAN_CONNECTION_ATTRIBUTES, internal::WlanApiDeleter>
GetConnectionAttributes() {
  ...
  if (result != ERROR_SUCCESS)
    return wlan_connection_attributes.Pass();
  base::scoped_ptr<WLAN_INTERFACE_INFO_LIST, internal::WlanApiDeleter> interface_list(
      interface_list_ptr);
  ...
  for (const IP_ADAPTER_ADDRESSES* adapter = adapters; adapter != NULL;
    ...
    if ((policy & EXCLUDE_HOST_SCOPE_VIRTUAL_INTERFACES) &&
        (strstr(adapter->AdapterName, "VMnet") != NULL || 
			strstr(base::SysWideToNativeMB(adapter->FriendlyName).c_str(), "VirtualBox") != NULL ||
			strstr(base::SysWideToNativeMB(adapter->FriendlyName).c_str(), "VMware") != NULL)) {
      continue;
    }
    ...
    for (IP_ADAPTER_UNICAST_ADDRESS* address = adapter->FirstUnicastAddress;
      ...
      if (family == AF_INET || family == AF_INET6) {
        ...
        if (endpoint.FromSockAddr(address->Address.lpSockaddr,
		  ...
		  NetworkInterface net_work_interface(adapter->AdapterName,
		  ...
		  net_work_interface.mac_address.resize(adapter->PhysicalAddressLength);
		  memcpy(&net_work_interface.mac_address.front(), adapter->PhysicalAddress, adapter->PhysicalAddressLength);
          networks->emplace_back(net_work_interface);
        }
      }
    }
  }
...
bool GetNetworkList(NetworkInterfaceList* networks, int policy) {
  ...
  base::scoped_ptr<char[]> buf(new char[len]);
...
class WifiOptionSetter : public ScopedWifiOptions {
 public:
  WifiOptionSetter(int options) {
    ...
    if (result != ERROR_SUCCESS)
      return;
	base::scoped_ptr<WLAN_INTERFACE_INFO_LIST, internal::WlanApiDeleter>
...
base::scoped_ptr<ScopedWifiOptions> SetWifiOptions(int options) {
  return base::scoped_ptr<ScopedWifiOptions>(new WifiOptionSetter(options));
}
