...
DB_BEGIN_DECLS
	...
	{
	public:
		static bool HasVersionTable(SQLiteDB* db){ return false; }
		static bool UpdateVersion(SQLiteDB* db, const TDBVersionType& version){ return true; }
		static TDBVersionType CheckVersion(SQLiteDB* db){ return TDBVersionType(); };
		static bool SetVersion(SQLiteDB* db, TDBVersionType version){ return true; };
		static bool CreateVersionTable(SQLiteDB* db){ return false; }
		static TDBVersionType GetInvalidVersionValue(){ return TDBVersionType(); }
		static TDBVersionType GetEmptyVersionValue(){ return TDBVersionType(); }
	};
	//一个版本号类型是int的特化
	template<>
	...
	public:
		static bool HasVersionTable(SQLiteDB* db){
			SQLiteStatement stmt;
			db->Query(stmt, "SELECT * FROM version");
			auto result = stmt.NextRow();
			return (result == SQLITE_ROW || result == SQLITE_DONE);
		}
		static bool UpdateVersion(SQLiteDB* db, const DBVersionType& version){
            std::string query_sql = nbase::StringPrintf("UPDATE OR ROLLBACK version SET version = %d", version);
			return db->Query(query_sql.c_str()) == SQLITE_OK;
		};
		static DBVersionType CheckVersion(SQLiteDB* db){
		...
		static bool SetVersion(SQLiteDB* db, DBVersionType version){
			std::string sql = nbase::StringPrintf("INSERT INTO version(version)VALUES(%d);", version);
			return db->Query(sql.c_str()) == SQLITE_OK;
		};
		static bool CreateVersionTable(SQLiteDB* db){
	...
	class DB_EXPORT DBPretreatment
	{
		...
		public:
			...
			virtual bool MoveFileX(const std::string &from_path, const std::string &to_path) = 0;
		...
		public:
			...
			bool DoBackup()
			{
				...
				if (file_system_->FilePathIsExist(db_path_, false))
				{
					...
					if (has_back_file)
						file_system_->MoveFileX(back_db_path_, back_db_file_bk);		
					auto backup_ret = false;
					{
						...
						else
						{
							if (file_system_->FilePathIsExist(back_db_file_bk, false))
							{
								...
								file_system_->MoveFileX(back_db_file_bk, back_db_path_);
							}
						}
			...
			bool DoRestore()
			{
				...
				if (file_system_->FilePathIsExist(back_db_path_, false))
				{
					...
					auto copy_ret = false;
					{
						...
						if (has_db_file)
							file_system_->MoveFileX(db_path_, db_file_bk);
						...
						else
						{
							if (file_system_->FilePathIsExist(db_file_bk, false))
							{
								...
								file_system_->MoveFileX(db_file_bk, db_path_);
							}
						}
						file_system_->UnLockDBFile();
					}
	...
	public:
		...
		typedef DBVersionOpration<DBVersionType> VersionOpration;
		typedef std::function<bool(DBVersionType)> DBUpdateFunc;
		typedef std::pair<DBVersionType, DBUpdateFunc> DBUpdateFunItem;
		...
		class FileSystemAutoUnLock {
		public:
			FileSystemAutoUnLock(FileSystem*file_system) :file_system_(file_system) { file_system_->LockDBFile(); }
			~FileSystemAutoUnLock() { file_system_->UnLockDBFile(); }
		private:
			FileSystem*file_system_;
		};
		class DB_EXPORT PretreatmentConfig
		{
		public:
			PretreatmentConfig() :
	...
	public:
		DBPretreatment() :
			file_system_(nullptr), ready_(false)
		{
		}
	...
	protected:
		...
		{			
			...
			if (ready_)
				return true;
			FileSystemAutoUnLock filesystemautounlock(file_system);
			file_system_ = file_system;
			try
			{
		...
		{
			SQLiteStatement stmt;
            std::string query_sql = nbase::StringPrintf("PRAGMA table_info(%s)", table.c_str());
			auto ret = db_.Query(stmt, query_sql.c_str());
			auto result = stmt.NextRow();
		...
		bool AddFieldToTable(const std::string& table, const std::string& field,const std::string& field_type) const
		{
			...
			{
				std::string sql = nbase::StringPrintf("ALTER TABLE %s ADD COLUMN %s %s", table.c_str(), field.c_str(), field_type.c_str());
				return db_.Query(sql.c_str()) == SQLITE_OK;
			}
			return true;
		}
	private:
		...
		{
			bool ret = false;
			int open_db_flg = SQLiteDB::modeReadWrite | SQLiteDB::modeCreate | SQLiteDB::modeSerialized;
	...
	private:		
		...
		DBUpdateFuncList updatefunctions_;
