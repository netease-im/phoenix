...
OSInfo::OSInfo()
  ...
  fnRtlGetVersion get_rtl_version;
  DWORD os_type;

  if (version_info.dwMajorVersion == 6 || version_info.dwMajorVersion == 10) {
    // Only present on Vista+.    
	get_rtl_version = reinterpret_cast<fnRtlGetVersion>(
	...
	try
	{
		if (get_rtl_version == nullptr) throw 1;
		VersionInformation.dwOSVersionInfoSize = sizeof(RTL_OSVERSIONINFOW);
		if (get_rtl_version(&VersionInformation) != S_OK) throw 1;
		version_info.dwMajorVersion = VersionInformation.dwMajorVersion;
		version_info.dwMinorVersion = VersionInformation.dwMinorVersion;
		version_info.dwBuildNumber = VersionInformation.dwBuildNumber;
		get_product_info(0, 0, 0, 0, &os_type);
	}
	catch (int)
	{
