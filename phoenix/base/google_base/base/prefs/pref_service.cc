...
base::scoped_ptr<base::DictionaryValue> PrefService::GetPreferenceValues() const {
  DCHECK(CalledOnValidThread());
    base::scoped_ptr<base::DictionaryValue> out(new base::DictionaryValue);
...
base::scoped_ptr<base::DictionaryValue> PrefService::GetPreferenceValuesOmitDefaults()
    const {
  DCHECK(CalledOnValidThread());
        base::scoped_ptr<base::DictionaryValue> out(new base::DictionaryValue);
  for (const auto& it : *pref_registry_) {
...
base::scoped_ptr<base::DictionaryValue>
...
void PrefService::SetUserPrefValue(const std::string& path,
    ...
    base::scoped_ptr<base::Value> owned_value(new_value);
  DCHECK(CalledOnValidThread());
