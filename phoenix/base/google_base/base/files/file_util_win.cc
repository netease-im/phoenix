...
#else
  if (GetMappedFileName(cp, file_view, mapped_file_path, kMaxPathLength))
  {
#define BUFSIZE 512
	  ...
	  if (GetLogicalDriveStrings(BUFSIZE - 1, szTemp))
	  {
		  ...
		  do
		  {
			  ...
			  *szDrive = *p;
			  ...
			  if (QueryDosDevice(szDrive, const_cast<WCHAR*>(szName.data()), MAX_PATH))
			  {
				  size_t uNameLen = szName.length();

				  if (uNameLen < MAX_PATH)
				  {
					  std::wstring mapped_file_path_temp(mapped_file_path);
					  bFound = mapped_file_path_temp.find(szName) == 0 && mapped_file_path_temp[uNameLen] == TEXT('\\');					
					  ...
					  if (bFound)
					  {
						  ...
						  StringCchPrintf(szTempFile,
							  ...
							  TEXT("%s%s"),
							  szDrive,
							  mapped_file_path + uNameLen);
						  StringCchCopyN(mapped_file_path, MAX_PATH + 1, szTempFile, _tcslen(szTempFile));
					  }
				  }
			  }
			  while (*p++);
